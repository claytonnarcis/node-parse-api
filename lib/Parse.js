...
Parse.prototype = {
  ...
  // get objects from class store based on objectId
  find: function (className, objectId, callback) {
      parseRequest.call(this, 'GET', '/1/classes/' + className + '/' + query, null, callback);
  },

  // get objects from class store based on query
  query: function (className, query, callback) {
	parseRequest.call(this, 'GET', '/1/classes/' + className, query, callback);
  },
...
function parseRequest(method, path, data, callback) {
  ...
  switch (method) {
    case 'GET':
	  // if it has data, encode is
      if (data) {
		body = data;
		
		// These headers are required to get GET data parse
